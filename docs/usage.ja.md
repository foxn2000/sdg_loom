...
models:
  - name: reasoning_model
    ...
    exclude_reasoning: false       # Reasoningを除外しない
    request_defaults:
      temperature: 0.7
      max_tokens: 8192
```
...
| パラメータ | デフォルト | 説明 |
|-----------|----------|------|
| `enable_reasoning` | `false` | Reasoning機能の有効化（**必須**：これがないとReasoning機能が動作しません） |
| `include_reasoning` | `true` | Reasoningを出力に含めるか |
| `exclude_reasoning` | `false` | Reasoningを内部利用のみにして出力から除外 |
| `reasoning_effort` | - | 推論レベル（`low`, `medium`, `high`, `xhigh`, `minimal`） |
| `reasoning_max_tokens` | - | Reasoning用の最大トークン数（オプション） |
...
SDG-LOOMは、高スループットのLLM推論ワークロードのための高度な最適化機能を提供します。これらの機能は、vLLMやSGLangバックエンドを使用する際に特に有効です。
...
SDG-LOOM Phase 2では、スケーラビリティとメモリ効率を向上させる高度な最適化機能を導入しました。これらの機能はすべて**オプトイン方式**で、デフォルトでは無効になっており、後方互換性を保証します。
