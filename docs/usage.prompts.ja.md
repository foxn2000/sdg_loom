...
MABEL（Model And Blocks Expansion Language）は、AIエージェントの処理フローをYAML形式で宣言的に記述できる言語仕様です。SDG-LOOMで使用され、複雑なAIワークフローを構造化して定義できます。
...
現在運用中のAIエージェントシステム（SDG-LOOM）の実装を参考に、指定されたロジックをMABELフロー形式で明確かつ動作保証のあるYAMLファイルとして作成してください。
